...
class RandomForest:
    ...
    def __train_test(self, name: str, option: str) -> None:
        ...
        self.__save_results(name, y_test, y_pred, labels)
    ...
    def __cross_validation_test(self, name: str) -> None:
        ...
        self.__save_results(name, self.labels, y_pred, labels)

    @staticmethod
    def __save_results(name: str, y_real: pd.Series, y_pred: pd.Series, labels: [str]) -> None:
        """
        Save results
        ...
        c_m_display.plot()
        plt.savefig(os.path.join("outputs", "graphs", "{}.png".format(name)))
        with open(os.path.join("outputs", "scores", "{}.csv".format(name)), "w", encoding="utf-8") as file:
            file.write(
                "Accuracy,{acc}\n"
